...
(def supported-agents #{"claude" "codex" "copilot" "grok" "kimi"})
(def red "\u001b[0;31m")
(def green "\u001b[0;32m")
(def yellow "\u001b[1;33m")
(def cyan "\u001b[0;36m")
          ...
          (if (or (str/blank? line) (str/starts-with? line "#"))
            ...
            (let [fields (str/split line #"\s+")]
              ...
              (let [[keyword role agent worktree & trailing] fields
                ...
                (when-not (supported-agents agent)
                ...
                (let [worktree-path (if (#{"none" "master"} worktree)
                      ...
                      row {:role role
                           ;; swarm <agent> overrides the conf agent column for
                           ;; every window; its conf extra-args are agent-specific
                           ;; so they are dropped in favor of the agent's baked defaults.
                           :agent (or (:agent-override ctx) agent)
                           :session (session-name-for-role role)
                           :display-name (display-name-for-role role)
                           :worktree-name worktree
                           :worktree-path worktree-path
                           :receive-mode receive-mode
                           :extra-args (if (:agent-override ctx) nil extra-args)}]
                  (recur (next lines)
                         (conj rows row)
                         (conj roles role)
  ...
  (let [role (:role row)
    ...
    (cond-> (str base
                (case agent
                  ...
                  ;; kimi rejects a positional prompt (treats it as a subcommand),
                  ;; has no --append-system-prompt-file, and -p exits after one
                  ;; reply. So it launches interactive with --yolo baked in and runs
                  ;; in the worktree via the cd in `base`; launch-role! then delivers
                  ;; the instruction via a second send-keys once the kimi TUI is
                  ;; ready. extra-args is nil on the swarm <agent> override path.
                  "kimi" (str "kimi --yolo " (extra-args-prefix row))))
      (= index 0)
...
(defn agent-instruction-line [prompt-file]
  ;; Collapse the multi-line instruction file into a single line so it can be
  ;; submitted to a TUI agent (kimi) as one message via send-keys + Enter.
  (-> (slurp (str prompt-file))
      (str/replace #"\s*\n\s*" " ")
      str/trim))

(defn tmux-capture [ctx target]
  (str/trim (:out (process/sh {:continue true}
                              "tmux" "-S" (:tmux-socket ctx) "capture-pane" "-p" "-t" target))))

(defn kimi-ready? [pane]
  ;; kimi's live status bar (the token-context readout) only renders once the
  ;; interactive input loop is accepting keys.
  (boolean (re-find #"context:\s*\d" pane)))

(defn wait-for-kimi-ready! [ctx target]
  ;; Poll the pane until kimi's TUI is interactive. Primary signal is the live
  ;; status bar; the quiescence fallback covers UI/version changes that move it.
  (let [timeout-ms (env-long "SWARMFORGE_KIMI_READY_TIMEOUT_MS" 30000)
        poll-ms 500
        deadline (+ (System/currentTimeMillis) timeout-ms)]
    (loop [prev nil stable 0]
      (let [pane (tmux-capture ctx target)]
        (cond
          (kimi-ready? pane) true
          (>= (System/currentTimeMillis) deadline) false
          :else (let [stable (if (and (seq pane) (= pane prev)) (inc stable) 0)]
                  (if (>= stable 4)
                    true
                    (do (Thread/sleep poll-ms)
                        (recur pane stable)))))))))

(defn send-kimi-instruction! [ctx target prompt-file]
  ;; kimi can't take the instruction as a CLI arg, so type it into the TUI once
  ;; ready, then submit with a plain Enter (kimi submits on unmodified Enter).
  (wait-for-kimi-ready! ctx target)
  (sh "tmux" "-S" (:tmux-socket ctx) "send-keys" "-t" target "-l" (agent-instruction-line prompt-file))
  (Thread/sleep 300)
...
(defn launch-role! [ctx index row]
  (let [session (:session row)
    ...
    (sh "tmux" "-S" (:tmux-socket ctx) "send-keys" "-t" target command "Enter")
    (when (= "kimi" (:agent row))
      (send-kimi-instruction! ctx target prompt-file))
...
(defn test-parse!
  ([root] (test-parse! root nil))
  ([root agent-override]
   (let [ctx (prepare-ctx (assoc (context root) :agent-override (some-> agent-override str/lower-case)))]
...
(defn run-main! [root agent-override]
  ...
  (check-dependency! "bb")
  (when (and agent-override (not (supported-agents agent-override)))
    (fail! (str red "Error:" reset " Unknown agent override '" agent-override
                "'. Supported agents: " (str/join ", " (sort supported-agents)))))
...
(defn -main [& args]
  (case (first args)
    "--test-parse" (test-parse! (or (second args) (System/getProperty "user.dir")) (nth args 2 nil))
    ...
    ;; First positional is now an optional agent override (swarm <agent>) that
    ;; replaces every conf window's agent column; the root is always the cwd.
    (run-main! (System/getProperty "user.dir") (some-> (first args) str/lower-case))))
